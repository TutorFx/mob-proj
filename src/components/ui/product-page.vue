  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 grid-rows-[max-content_1fr] lg:grid-rows-[1fr] items-start lg:items-center mb-6 lg:mb-0">
    ...
    <div class="grid items-start lg:items-center min-h-full">
      <div class="grid gap-3 md:gap-6">
        ...
        <h2 class="text-4xl font-bold grid grid-flow-col justify-start gap-6 items-center"><span>{{ productdata?.name }}</span> <span @click="startShare" v-if="isSupported" class="btn btn-ghost btn-sm btn-circle"><Icon name="mdi:share-variant" size="24" /></span>
      </h2>
