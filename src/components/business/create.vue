        ...
        <div class="m-auto text-white border rounded-xl w-4xl overflow-hidden border-zinc-700">
          ...
          <div class="p-6 bg-zinc-900 border-y border-zinc-700 grid gap-4">
            <label class="text-zinc-400">
              ...
              <input v-model="business.name" type="text" class="bg-black border-zinc-700 rounded w-full">
            </label>
            <label class="text-zinc-400">
              <div class="mb-3">
                Link da empresa
              ...
              <div class="grid grid-flow-col rounded border border-zinc-700">
                ...
                <input v-model="business.slug" type="text" class="bg-black border-black w-full rounded-r text-sm" placeholder="minha-empresa">
              </div>
            </label>
          </div>
          <div class="p-6 bg-zinc-900 flex justify-between">
            <button @click.prevent="businessStore.closePopup()"
            ...
            <button @click.prevent="createBusiness"
              ...
              <div v-if="businessStore.$state.scopedBusinessesFetching" class="absolute inset-0 flex items-center justify-center bg-purple-200 text-black cursor-wait">
                <ui-spinner />
              </div>
              <div>Continue</div>
            </button>
...
const business = ref(starterBusiness)
const createBusiness = async() => {

  if (businessStore.$state.scopedBusinessesFetching) return;
  const {status, data} = await businessStore.create(business.value)

  if(status != 200) return;

  router.push({ path: `/dashboard/${data?.id}`})
