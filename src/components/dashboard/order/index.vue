      ...
      <div class="grid grid-cols-[max-content_1fr] gap-2 lg:gap-6">
        ...
        <div class="grid gap-2">
          <nuxt-link
            ...
            :to="{
              name: 'dashboard-id-fechamento-checkoutid',
              params: { id: order.Business.id, checkoutid: order.id },
            }"
          >
            <div class="grid grid-flow-col items-center justify-start gap-1">
              <div class="truncate font-bold">Pedido</div>
              <div
                class="w-24 truncate rounded bg-base-200 px-2"
                :title="order.id"
              >
          ...
          <div
            v-if="order?.ProductOnOrder.length > 0"
            class="grid gap-3 overflow-hidden"
          >
            <dashboard-order-product
              class="grid grid-cols-[max-content_1fr] gap-3"
              :item="order?.ProductOnOrder[0]"
            />
            <div
              v-if="order?.ProductOnOrder.length > 1 && isVisible"
              class="grid gap-3"
            >
              <dashboard-order-product
            ...
            <div v-if="order?.ProductOnOrder.length > 1">
              ...
              >
                Mais {{ order?.ProductOnOrder.length - 1 }} Produtos
                <Icon
                  :name="!isVisible ? 'mdi:chevron-down' : 'mdi:chevron-up'"
                />
              </button>
            </div>
          </div>
