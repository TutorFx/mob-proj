...
export default defineEventHandler(async (event) => {
  ...
  if (stripeEvent.type?.startsWith("customer.subscription")) {
    ...
    if (subscription.status === "active") {
      ...
      const currentPeriodEnds: Date = new Date(
        subscription.current_period_end * 1000,
      );
      currentPeriodEnds.setDate(
        currentPeriodEnds.getDate() + config.subscriptionGraceDays,
      );

      console.log(
        `updating stripe sub details subscription.current_period_end:${subscription.current_period_end}, subscription.id:${subscription.id}, stripe_product_id:${stripeProductId}`,
      );
