    ...
    <div class="container grid gap-6 pb-6 md:grid-cols-2">
      <div>
        <div class="grid gap-3 py-6">
          ...
          <div>
            Seu pedido já está em { etapa }, {{ data?.Business.name }} entrará
            em contato com você via Whatsapp para confirmar seu pedido.
          </div>
          <div class="border-b" />
          <div class="grid gap-6 xl:grid-cols-4">
            ...
            <ul>
              <li class="text-neutral">Endereço</li>
              <li>
                {{ data?.address?.bairro }}
              </li>
            </ul>
...
const formatted_message = computed(() => {
  const greating = encodeURIComponent(`Olá, ${useGreeting()}.`)
    .replace(/'/g, "%27")
    .replace(/"/g, "%22");
  const message = encodeURIComponent(
    `Eu fiz o pedido #${data?.id} em ${data?.Business.name} e gostaria de dar continuidade no meu atendimento por aqui.`,
  )
    .replace(/'/g, "%27")
    .replace(/"/g, "%22");
